...
export class InvoiceService {
  ...
  protected convertDateFromClient(invoice: IInvoice): IInvoice {
    const copy: IInvoice = Object.assign({}, invoice, {
      dateOfCreation: invoice.dateOfCreation && invoice.dateOfCreation.isValid() ? invoice.dateOfCreation.toJSON() : undefined,
      createdAt: invoice.createdAt && invoice.createdAt.isValid() ? invoice.createdAt.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((invoice: IInvoice) => {
        invoice.dateOfCreation = invoice.dateOfCreation ? moment(invoice.dateOfCreation) : undefined;
        invoice.createdAt = invoice.createdAt ? moment(invoice.createdAt) : undefined;
      });
    }
    return res;
  }
