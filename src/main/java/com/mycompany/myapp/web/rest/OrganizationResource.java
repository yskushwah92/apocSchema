...
public class OrganizationResource {
    ...
    public List<Organization> getAllOrganizations(@RequestParam(required = false) String filter) {
        if ("vendor-is-null".equals(filter)) {
            log.debug("REST request to get all Organizations where vendor is null");
            return StreamSupport
                .stream(organizationRepository.findAll().spliterator(), false)
                .filter(organization -> organization.getVendor() == null)
                .collect(Collectors.toList());
        }
        if ("purchaseorderline-is-null".equals(filter)) {
            log.debug("REST request to get all Organizations where purchaseOrderLine is null");
            return StreamSupport
                .stream(organizationRepository.findAll().spliterator(), false)
                .filter(organization -> organization.getPurchaseOrderLine() == null)
                .collect(Collectors.toList());
        }
